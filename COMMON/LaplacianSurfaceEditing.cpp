  ...
  const std::vector<int>& _fixed_verts_idxs)
{
  ...
  m_coef = 1.0f;
  m_coef_weak = 0.0001f * m_times;

  m_fixed_verts_idxs = _fixed_verts_idxs;
  m_num_fixed_verts = m_fixed_verts_idxs.size();
  ...
  const std::vector<Eigen::Vector3f>& _fixed_positions)
{
  ...
  {
    // keep original delta norm
    Eigen::VectorXf delta_norm = Eigen::VectorXf(m_num_verts);
    for (int i = 0; i < m_num_verts; ++i)
    {
      delta_norm(i) = m_delta.row(i).norm();
    }

    // iterate
    Eigen::MatrixXf result = m_verts;
    for (int i = 0; i < m_times; ++i)
    {
      for (int j = 0; j < m_num_verts; ++j)
      {
        EVec3f dj = delta_norm(j) * m_mesh.m_vNorms[j];
        m_delta.row(j) = dj.transpose();
      }
