...
//#define DEBUG_MODE_FIXMASK


using namespace RoiPainter4D;


ModeFixMask::ModeFixMask() :
...
{
  std::cout << "ModeFixMask...\n";
  m_bL = m_bR = m_bM = false;
  std::cout << "ModeFixMask done\n";
}
...
void ModeFixMask::StartMode()
{
  std::cout << "ModeFixMask...startMode----------\n";
  m_bL = m_bR = m_bM = false;
...
void ModeFixMask::LBtnDown(const EVec2i& p, OglForCLI* ogl)
{
  m_bL = true;
...
void ModeFixMask::LBtnUp(const EVec2i& p, OglForCLI* ogl)
{
  m_bL = false;
  const int frame_idx = formVisParam_getframeI();
...
void ModeFixMask::RBtnDown(const EVec2i& p, OglForCLI* ogl)
{
  m_bR = true;
  const int frame_idx = formVisParam_getframeI();
...
void ModeFixMask::RBtnUp(const EVec2i& p, OglForCLI* ogl)
{
  m_bR = false;
...
void ModeFixMask::MBtnDown(const EVec2i& p, OglForCLI* ogl)
{
  m_bM = true;
...
void ModeFixMask::MBtnUp(const EVec2i& p, OglForCLI* ogl)
{
  m_bM = false;
  ogl->BtnUp();
  formMain_RedrawMainPanel();
...
void ModeFixMask::MouseMove(const EVec2i& p, OglForCLI* ogl)
{
...
void ModeFixMask::LBtnDclk(const EVec2i& p, OglForCLI* ogl)
{

}


void ModeFixMask::RBtnDclk(const EVec2i& p, OglForCLI* ogl)
{
  const int frame_idx = formVisParam_getframeI();
...
void ModeFixMask::MBtnDclk(const EVec2i& p, OglForCLI* ogl) {}


void ModeFixMask::KeyDown(int nChar)
{
...
void ModeFixMask::KeyUp(int nChar) {}


void ModeFixMask::DrawScene(
  const EVec3f& cuboid,
  const EVec3f& camP,
  const EVec3f& camF)
{
...
void ModeFixMask::SetCPSize()
{
  const int cp_size = FormFixMask_GetCPSize();
...
void ModeFixMask::ConvertMaskToMesh()
{
  const int scale = FormFixMask_GetMCScale();
...
void ModeFixMask::ConvertMeshToMask()
{
  const int frame_idx = formVisParam_getframeI();
  for (int i = 0; i < ImageCore::GetInst()->GetNumFrames(); ++i) {
    if (i == frame_idx) continue;
    m_mask_mesh.UpdateMask(i);
  }
...
void ModeFixMask::ReloadMesh(const int _frame_idx)
{
  if (!m_exist_mesh || !m_mask_mesh.is_initialized) return;
...
void ModeFixMask::CopyFromPrevFrame()
{
  const int frame_idx = formVisParam_getframeI();
...
void ModeFixMask::CopyStrokesToAllFrame()
{
  const int frame_idx = formVisParam_getframeI();
  for (int i = 0; i < ImageCore::GetInst()->GetNumFrames(); ++i) {
    if (i == frame_idx) continue;
...
void ModeFixMask::SetShowOnlySelectedStroke()
{
  m_show_only_selected_stroke = FormFixMask_GetShowOnlySelectedStroke();
  formMain_RedrawMainPanel();
  formMain_ActivateMainForm();
}
...
void ModeFixMask::SaveState(const std::string& _fpath, const std::set<int>& _set_frame_idx)
{
...
void ModeFixMask::LoadState(const std::string& _fpath, const std::set<int>& _set_frame_idx)
{
  enum class E_State
  {
    Init,
...
void ModeFixMask::ShareSelectedStroke()
{
  const int frame_idx = formVisParam_getframeI();
  const int num_frames = ImageCore::GetInst()->GetNumFrames();
...
void ModeFixMask::UnshareSelectedStroke()
{
  const int frame_idx = formVisParam_getframeI();
  DeformationStrokes& dstrokes = m_strokes[frame_idx];
...
void ModeFixMask::LockSelectedStroke()
{
  const int frame_idx = formVisParam_getframeI();
  DeformationStrokes& dstrokes = m_strokes[frame_idx];
...
void ModeFixMask::UnlockSelectedStroke()
{
  const int frame_idx = formVisParam_getframeI();
  DeformationStrokes& dstrokes = m_strokes[frame_idx];
...
void ModeFixMask::UpdateSharedStroke()
{
  const int frame_idx = formVisParam_getframeI();
  const int num_frames = ImageCore::GetInst()->GetNumFrames();
